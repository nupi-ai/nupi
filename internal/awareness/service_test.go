...
func TestStartCreatesDirectories(t *testing.T) {
	...
	expected := []string{
		...
		filepath.Join(dir, "awareness", "memory", "journals"),
		filepath.Join(dir, "awareness", "memory", "journals", "archives"),
		filepath.Join(dir, "awareness", "memory", "journals", "logs"),
		filepath.Join(dir, "awareness", "memory", "conversations"),
		filepath.Join(dir, "awareness", "memory", "conversations", "archives"),
		filepath.Join(dir, "awareness", "memory", "conversations", "logs"),
		filepath.Join(dir, "awareness", "memory", "topics"),
		filepath.Join(dir, "awareness", "memory", "projects"),
	}

	// Verify old directories are NOT created.
	oldDirs := []string{
...
func TestServiceSearch(t *testing.T) {
	...
	// Create a file in the memory directory structure.
	dataDir := filepath.Join(dir, "awareness", "memory", "conversations")
	if err := os.MkdirAll(dataDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dataDir, "2026-02-19.md"), []byte("## Test\n\nSearchable content here."), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestServiceHasEmbeddingsWithProvider(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dataDir, "test.md"), []byte("## Test\n\nContent."), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestEnsureDirectoriesCleansStaleTemp(t *testing.T) {
	...
	// Pre-create the conversations directory with a stale .tmp file.
	convDir := filepath.Join(dir, "awareness", "memory", "conversations")
	if err := os.MkdirAll(convDir, 0o755); err != nil {
		t.Fatal(err)
	}
	staleTmp := filepath.Join(convDir, "2026-02-19.md.tmp")
...
// TestEnsureDirectoriesCleansJournalsTmp verifies that stale .tmp files in the
// journals/ directory are cleaned up on service start.
func TestEnsureDirectoriesCleansJournalsTmp(t *testing.T) {
	dir := t.TempDir()

	journalDir := filepath.Join(dir, "awareness", "memory", "journals")
	if err := os.MkdirAll(journalDir, 0o755); err != nil {
		t.Fatal(err)
	}
	staleTmp := filepath.Join(journalDir, "rolling.md.tmp")
	if err := os.WriteFile(staleTmp, []byte("partial write"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(staleTmp); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("expected stale .tmp file in journals/ to be removed, got err: %v", err)
	}
}

// TestEnsureDirectoriesCleansProjectScopedTmp verifies that stale .tmp files in
// project-scoped directories (conversations/, journals/, topics/) are cleaned up.
func TestEnsureDirectoriesCleansProjectScopedTmp(t *testing.T) {
	dir := t.TempDir()

	projectsDir := filepath.Join(dir, "awareness", "memory", "projects", "myapp")
	for _, subdir := range []string{"journals", "conversations", "topics"} {
		...
		staleTmp := filepath.Join(subPath, "stale.md.tmp")
		if err := os.WriteFile(staleTmp, []byte("partial write"), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	...
	for _, subdir := range []string{"journals", "conversations", "topics"} {
		staleTmp := filepath.Join(projectsDir, subdir, "stale.md.tmp")
		if _, err := os.Stat(staleTmp); !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("expected stale .tmp file in projects/myapp/%s/ to be removed, got err: %v", subdir, err)
		}
	}
}
