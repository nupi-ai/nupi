...
func TestJSPlugin_Summarize(t *testing.T) {
	skipIfNoBun(t)

	tmpDir := t.TempDir()
	pluginPath := filepath.Join(tmpDir, "summarizer.js")
	pluginCode := `
module.exports = {
  name: "summarizer",
  commands: ["test"],
...
`
	...
	// Verify capability flag
	if !plugin.HasSummarize {
		t.Error("expected HasSummarize to be true")
	}

	// Test summarization of long output
	longOutput := "line1\nline2\nline3\nline4\nline5\nline6\nline7"
	summarized, err := plugin.Summarize(ctx, rt, longOutput)
	if err != nil {
		t.Fatalf("Summarize() error = %v", err)
	}
	expected := "line1\nline2\n...\nline6\nline7"
	if summarized != expected {
		t.Errorf("Summarize() = %q, want %q", summarized, expected)
	}

	// Test short output (no summarization)
	shortOutput := "line1\nline2"
	summarized, err = plugin.Summarize(ctx, rt, shortOutput)
	if err != nil {
		t.Fatalf("Summarize() error = %v", err)
	}
	if summarized != shortOutput {
		t.Errorf("Summarize() = %q, want original %q", summarized, shortOutput)
	}
}

func TestJSPlugin_Summarize_Fallback(t *testing.T) {
	skipIfNoBun(t)

	// Plugin without summarize function
	tmpDir := t.TempDir()
	pluginPath := filepath.Join(tmpDir, "no-summarize.js")
	pluginCode := `
module.exports = {
  name: "no-summarize",
  commands: ["test"],
...
`
	...
	// Verify capability flag is false
	if plugin.HasSummarize {
		t.Error("expected HasSummarize to be false")
	}

	// Should return original output when summarize not implemented
	longOutput := "line1\nline2\nline3\nline4\nline5\nline6\nline7"
	summarized, err := plugin.Summarize(ctx, rt, longOutput)
	if err != nil {
		t.Fatalf("Summarize() error = %v", err)
	}
	if summarized != longOutput {
		t.Errorf("Summarize() = %q, want original %q", summarized, longOutput)
	}
}
