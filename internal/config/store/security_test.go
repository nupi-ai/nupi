...
func TestSecuritySettingsEncryptedAtRest(t *testing.T) {
	...
	var rawValue string
	err = db.QueryRow(`SELECT ` + securitySettingsValueColumn + ` FROM security_settings WHERE key = 'test_key'`).Scan(&rawValue)
	if err != nil {
...
func TestOpenRWMigratesPlaintextToEncrypted(t *testing.T) {
	...
	var rawValue string
	err = rawDB.QueryRow(
		`SELECT ` + securitySettingsValueColumn + ` FROM security_settings WHERE key = 'legacy_key' AND instance_name = 'default' AND profile_name = 'default'`,
	).Scan(&rawValue)
	if err != nil {
		t.Fatalf("raw query: %v", err)
	}
...
func TestOpenRWMigratesPrefixCollisionCorrectly(t *testing.T) {
	...
	var rawValue string
	err = rawDB.QueryRow(
		`SELECT ` + securitySettingsValueColumn + ` FROM security_settings WHERE key = 'collision_key' AND instance_name = 'default' AND profile_name = 'default'`,
	).Scan(&rawValue)
	if err != nil {
		t.Fatalf("raw query: %v", err)
	}
...
func TestOpenWithMockKeychainDualWrites(t *testing.T) {
	...
	for k, plaintext := range secrets {
		...
		err = s.db.QueryRowContext(ctx,
			`SELECT `+securitySettingsValueColumn+` FROM security_settings WHERE key = ? AND instance_name = ? AND profile_name = ?`,
